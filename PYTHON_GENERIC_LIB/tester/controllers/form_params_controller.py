...
class FormParamsController(BaseController):
    ...
    def send_multiple_files(self,
        ...
        Args:
            file (typing.BinaryIO): TODO: type description here.
            file_1 (typing.BinaryIO): TODO: type description here.
        ...
        if isinstance(file_1, FileWrapper):
            file_1_wrapper = file_1.file_stream
            file_1_content_type = file_1.content_type
        else:
            file_1_wrapper = file_1
            file_1_content_type = 'application/octet-stream'

        # Prepare files
        _files = {
            'file': (file_wrapper.name, file_wrapper, file_content_type),
            'file1': (file_1_wrapper.name, file_1_wrapper, file_1_content_type)
        }
    ...
    def send_mixed_array(self,
        ...
        if isinstance(options.get('file', None), FileWrapper):
            file_wrapper = options.get('file', None).file_stream
            file_content_type = options.get('file', None).content_type
        else:
            file_wrapper = options.get('file', None)
            file_content_type = 'application/octet-stream'
